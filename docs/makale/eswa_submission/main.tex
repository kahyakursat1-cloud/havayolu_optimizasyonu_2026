...
\begin{abstract}
Disruption recovery operates under uncertainty: crew locations are imprecise, weather forecasts erroneous, and passenger connections cascade-fragile. Recovery decisions made minutes after disruption must balance optimality with robustness to uncertain outcomes. Classical exact solvers (CP-SAT) guarantee nominal optimality but timeout on moderately-large instances, especially with uncertainty quantification; metaheuristics run fast but risk regulatory violations or brittle solutions failing under adverse conditions. This paper presents a hybrid framework that reveals the true value of combining exact and evolutionary methods: on \emph{nominal deterministic instances}, CP-SAT dominates; on \emph{stochastic uncertain scenarios}, warm-started QIGA explores a robust feasible region that CP-SAT cannot reach within time budget, discovering solutions that remain feasible across multiple disruption outcomes while improving profit. We prove a \emph{warm-start safety guarantee} (Proposition 1) ensuring QIGA refinement maintains feasibility with high probability. On deterministic instances (300 flights, no uncertainty), CP-SAT achieves global optimality in <1ms. On stochastic instances (50-100 flights with crew location, weather, and passenger connection uncertainty), CP-SAT times out, but warm-started QIGA delivers robust near-optimal solutions in <5 seconds while maintaining regulatory compliance. SHAP-based explainability maps constraints to human-readable decision rationales, satisfying EASA AMC 20-42 transparency. Validation across both deterministic and stochastic domains demonstrates that the hybrid framework solves a real problem: robust recovery under uncertainty at scales where exact methods alone are insufficient.
\end{abstract}
...
(1)~\textbf{A robust hybrid optimization framework} combining CP-SAT (nominal feasibility) with warm-started QIGA (robust exploration under uncertainty). The framework balances \emph{optimality under nominal conditions} with \emph{robustness to disruption uncertainty} (crew location, weather forecast, passenger connection viability).

(2)~\textbf{Formal warm-start safety guarantee (Proposition~\ref{prop:warm})} proving that initializing a genetic algorithm from a CP-SAT solution ensures monotonic profit improvement with high probability while maintaining feasibility---a theoretical foundation for hybrid methods in regulated optimization.

(3)~\textbf{An EASA-compliant framework} enforcing crew duty regulations (CAT.OP.MPA.210) and demonstrating that the hybrid approach outperforms CP-SAT-only on robust instances where uncertainty is high (CP-SAT times out, QIGA explores feasible region).
...
(5)~\textbf{Comprehensive validation:} Deterministic (IATA-calibrated 300-flight baseline) and stochastic (uncertain 50-100 flight scenarios) experiments demonstrating when CP-SAT dominates (nominal) vs. when hybrid wins (high uncertainty).
