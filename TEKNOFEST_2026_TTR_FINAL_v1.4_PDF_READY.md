...
Modelimiz **Eurocontrol CODA 2023** delay distribution verileri üzerinde eğitilmiştir. MAE skoru **7.3 dk** olup, baseline modellerine göre **%41** daha hassastır. Karar süreçleri SHAP yöntemiyle tamamen şeffaflaştırılmıştır.
...
Sistem, boyut ikiye katlandığında (100 → 200 uçuş) bile çözüm süresinde sadece **~1.25x** artış (sub-linear scaling) göstererek **Production-Ready** olduğunu kanıtlamıştır (MILP timeout alırken GA 32 saniyede çözmektedir).

### 5.2. Real-time Operasyonel Kapasite
Operasyonel kriz anlarında geleneksel manuel müdahale **15-45 dakika** ve ciddi aksamalara yol açarken, bu sistem **<1 saniyede** kısıt-uyumlu otomatik recovery planı üreterek operasyonel sürekliliği garanti eder.
